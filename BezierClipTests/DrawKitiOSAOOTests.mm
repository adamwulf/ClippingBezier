...
-(void) setUp{
    ...
    [complexShape addCurveToPoint:CGPointMake(218.500000,376.000000) controlPoint1:CGPointMake(265.774200,396.677216) controlPoint2:CGPointMake(245.152649,385.690552)];
...
- (void)testExample{
    ...
    struct Bezier someCurve = {Point2D(100.0,50.0),Point2D(600.0,850.0),Point2D(370.0,80.0),Point2D(570.0,520.0)};
    
    ::std::vector< ::std::pair<double, double> > 								result;
    __block ::std::back_insert_iterator< ::std::vector< ::std::pair<double, double> > > ii(result);

    __block int found = 0;
    
    [complexShape iteratePathWithBlock:^(CGPathElement element, NSUInteger indx){
        if(element.type == kCGPathElementCloseSubpath){
            // noop
        }else{
            struct Bezier otherCurve = {};
            if(element.type == kCGPathElementAddCurveToPoint){
                otherCurve = {Point2D(lastPoint.x,lastPoint.y),
                    Point2D(element.points[0].x,element.points[0].y),
                    Point2D(element.points[1].x,element.points[1].y),
                    Point2D(element.points[2].x,element.points[2].y)};
            }else if(element.type == kCGPathElementAddLineToPoint){
                otherCurve = {Point2D(lastPoint.x,lastPoint.y),
                    Point2D(lastPoint.x,lastPoint.y),
                    Point2D(element.points[0].x,element.points[0].y),
                    Point2D(element.points[0].x,element.points[0].y)};
            }
            ...
            if(element.type != kCGPathElementMoveToPoint){
                struct Bezier c1( someCurve );
                struct Bezier c2( otherCurve );
                
                clipBezier(ii, (double)0.01, c1, c2);
