...
CGPoint diffPoints(CGPoint a, CGPoint b){
...
CGPoint V2ScaleII(CGPoint v, double s){
...
CGPoint NearestPointOnCurve(CGPoint P, CGPoint* V, double* t)
{
    ...
    /* Compare distances of P to all candidates, and to t=0, and t=1 */
    {
        ...
        dist = V2SquaredLength(CGPointDiff(P, V[0]));
        
        /* Find distances for candidate points	*/
        for (i = 0; i < n_solutions; i++) {
            p = Bezier(V, DEGREE, t_candidate[i], NULL, NULL);
            new_dist = V2SquaredLength(CGPointDiff(P, p));
            if (new_dist < dist) {
                dist = new_dist;
                t_out = t_candidate[i];
            }
        ...
        new_dist = V2SquaredLength(CGPointDiff(P, V[DEGREE]));
        if (new_dist < dist) {
            dist = new_dist;
            t_out = 1.0;
        }
...
//        printf("t : %4.12f\n", t);
    ...
    return (Bezier(V, DEGREE, t_out, NULL, NULL));
}
