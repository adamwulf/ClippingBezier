...
CG_INLINE CGPoint
CGPointLerp(CGPoint a, CGPoint b, CGFloat t)
{
...
+ (NSArray<NSValue *> *)altFindIntersectionsBetweenBezier:(CGPoint[4])bez andLineFrom:(CGPoint)a1 to:(CGPoint)a2
{
    ...
    cx = ax + bx;
    cy = ay + by;
    c1 = CGVectorMake(cx, cy);

    c0 = CGVectorMake(p1.x, p1.y);

    // Convert line to normal form: ax + by + c = 0
    // Find normal to line: negative inverse of original line's slope
    n = CGVectorMake(a1.y - a2.y, a2.x - a1.x);

    // Determine new c coefficient
    CGFloat cl = a1.x * a2.y - a2.x * a1.y;
    ...
    for (int i = 0; i < roots.count; i++) {
        ...
        if (0 <= t && t <= 1) {
            ...
            CGPoint p6 = CGPointLerp(p2, p3, t);
            CGPoint p7 = CGPointLerp(p3, p4, t);

            CGPoint p8 = CGPointLerp(p5, p6, t);
            CGPoint p9 = CGPointLerp(p6, p7, t);

            CGPoint p10 = CGPointLerp(p8, p9, t);
